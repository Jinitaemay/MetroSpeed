...
> **记忆版本**：v24
> **最后更新**：2026-07-02
> **对应阶段**：项目目标写回文档；AppGallery 上架通过；重力传感器分析完成（NO-GO）；隧道定位机制确认；README 时间线精简
...
13. **系统重力传感器在地铁场景吃加速度**：初步对比（2 条 v13 记录）显示，`--use-sys-gravity` 模式下地铁记录中位速度从 40.9 km/h 塌到 0.6 km/h——系统重力将地铁起步加速吃进重力分量，与 4 月踩坑一致。驾车记录系统重力模式 max speed 从 909→80 km/h，看似改善，但这是因为系统重力隐式包含了系统融合的 IMU+GNSS 推算结果，不是独立的纯惯性——拿系统跟系统比不是真正的独立验证。
...
- **replay_estimator.py 分析层开关**：`--use-gyro-gravity`（陀螺仪重力追踪，已验证失败）、`--use-sys-gravity`（系统重力替代自估重力，分析中）。这些是分析层参数，不改变 SpeedEstimator 默认行为
...
| 07-02 | 重力传感器分析 | 完成 `--use-sys-gravity` 分析工具（replay_estimator.py）；初步对比：地铁场景系统重力吃加速度（中位速度 40.9→0.6 km/h），驾车场景"改善"实为系统自比不可信；确认鸿蒙系统隧道定位机制为 IMU 惯性推算（satFix/satCount/accuracy 恒定特征），非真实 GNSS |
...
18. 初步对比完成：系统重力在地铁场景吃加速度（NO-GO 信号），驾车场景"改善"不可信（系统自比）
19. 确认鸿蒙系统隧道定位机制为 IMU 惯性推算，非真实 GNSS
20. 关闭重力传感器接入 spec：初步对比确认系统重力在地铁场景吃加速度（中位速度 40.9→0.6 km/h），驾车场景"改善"来自系统融合 IMU+GNSS 推算不可信，结论 NO-GO

**待执行任务（按优先级）**：
1. 🟡 分析龙阳路→张江高科长期无校准漂移问题（max=80km/h，无 GNSS 有效修复）
2. 🟢 多语言支持（英文）
3. 🟢 后台长时记录稳定性测试
4. 🟢 历史记录管理界面
...
20. **系统重力传感器不可用于纯惯性测速**：地铁场景吃加速度（中位速度 40.9→0.6 km/h），驾车场景的"改善"来自系统融合的 IMU+GNSS 推算，不是独立的纯惯性。拿系统重力做对比等于拿系统跟系统比。
